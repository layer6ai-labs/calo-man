...
def plot_layer_comparison(hlf_class, data, reference_class, reference_data, arg, show=False):
    """ plots showers of data and reference next to each other, for comparison """
...
def plot_Etot_Einc_discrete(hlf_class, reference_class, arg):
    ...
    for i in range(len(target_energies) - 1):
        ...
        counts_ref, _, _ = ax.hist(reference_class.GetEtot()[which_showers_ref] /\
                                   ...
                                   histtype='stepfilled', alpha=0.2, linewidth=2., color='#5EC962')
...
def plot_Etot_Einc(hlf_class, reference_class, arg):
    ...
    counts_data, _, _ = plt.hist(hlf_class.GetEtot() / hlf_class.Einc.squeeze(), bins=bins,
                                 label='generated', histtype='step', linewidth=3., alpha=1.,
                                 density=True, color='#3B528B')
    plt.title("Histogram of Deposited Energy to Incident Energy Ratio")
    plt.xlim(0.5, 1.5)
...
def plot_E_layers(hlf_class, reference_class, arg):
    ...
    for key in hlf_class.GetElayers().keys():
        ...
        counts_ref, bins, _ = plt.hist(reference_class.GetElayers()[key], bins=bins,
                                       ...
                                       alpha=0.2, linewidth=2., color='#5EC962')
        counts_data, _, _ = plt.hist(hlf_class.GetElayers()[key], label='generated', bins=bins,
                                     histtype='step', linewidth=3., alpha=1., density=True, color='#3B528B')
...
def plot_ECEtas(hlf_class, reference_class, arg):
    ...
    for key in hlf_class.GetECEtas().keys():
        if arg.dataset in ['electrons2', 'electrons3', '2', '3']:
            lim = (-30., 30.)
        elif key in [12, 13]:
            lim = (-500., 500.)
        else:
...
def plot_eperlayer_comparison(gen_eperlayer, ref_eperlayer, args, show=False):
    """ plots energy deposited in each layer """
    ...
    xml_filename = os.path.join(os.path.dirname(os.path.dirname(
        os.path.realpath(__file__))), "evaluators", "binning", f"binning_dataset_{args.dataset}.xml")

    xml = XMLHandler(particle, filename=xml_filename)
    relevantLayers = xml.GetRelevantLayers()

    for idx,layer in enumerate(relevantLayers):
        plt.figure(figsize=(6, 6))
        if args.x_scale == 'log':
            bins = np.logspace(np.log10(args.min_energy),
                               np.log10(ref_eperlayer[:,idx].max()),
                               40)
        else:
            bins = 40
        counts_ref, bins, _ = plt.hist(ref_eperlayer[:,idx], bins=bins,
                                       label='reference', density=True, histtype='stepfilled',
                                       alpha=0.2, linewidth=2., color='#5EC962')
        counts_data, _, _ = plt.hist(gen_eperlayer[:,idx], label='generated', bins=bins,
                                     histtype='step', linewidth=3., alpha=1., density=True, color='#3B528B')
        plt.title("Energy deposited in layer {}".format(layer))
        plt.xlabel(r'$E$ [MeV]')
        plt.yscale('log')
        plt.xscale('log')
        plt.legend(fontsize=20)
        plt.tight_layout()
        if args.mode in ['all', 'hist-p', 'hist']:
            filename = os.path.join(args.output_dir, 'E_layer_{}_dataset_{}.png'.format(
                layer,
                args.dataset))
            plt.savefig(filename, dpi=300, bbox_inches='tight')
        if args.mode in ['all', 'hist-chi', 'hist']:
            seps = _separation_power(counts_ref, counts_data, bins)
            print("Separation power of EPL E layer {} histogram: {}".format(layer, seps))
            with open(os.path.join(args.output_dir, 'histogram_epl_chi2_{}.txt'.format(args.dataset)),
                      'a') as f:
                f.write('E layer {}: \n'.format(layer))
                f.write(str(seps))
                f.write('\n\n')
...
def plot_Etot_Einc_comparison(gen_eperlayers, gen_einc, ref_eperlayers, ref_einc, args):
    """ plots Etot normalized to Einc histogram """

    ref_etot = np.sum(ref_eperlayers,axis=1)
    gen_etot = np.sum(gen_eperlayers,axis=1)

    bins = np.linspace(0.5, 1.5, 101)
    plt.figure(figsize=(6, 6))
    counts_ref, _, _ = plt.hist(ref_etot / ref_einc.squeeze(),
                                bins=bins, label='reference', density=True,
                                histtype='stepfilled', alpha=0.2, linewidth=2.)
    counts_data, _, _ = plt.hist(gen_etot / gen_einc.squeeze(), bins=bins,
                                 label='generated', histtype='step', linewidth=3., alpha=1.,
                                 density=True)
    plt.xlim(0.5, 1.5)
    plt.xlabel(r'$E_{tot} / E_{inc}$')
    ...
    plt.tight_layout()
    if args.mode in ['all', 'hist-p', 'hist']:
        filename = os.path.join(args.output_dir, 'Etot_Einc_dataset_{}.png'.format(args.dataset))
        plt.savefig(filename, dpi=300)
    if args.mode in ['all', 'hist-chi', 'hist']:
        seps = _separation_power(counts_ref, counts_data, bins)
        print("Separation power of Etot / Einc histogram: {}".format(seps))
...
def plot_Etot_Einc_discrete_comparison(gen_eperlayers, gen_einc, ref_eperlayers, ref_einc, arg):
    """ plots Etot normalized to Einc histograms for each Einc in ds1 """
    # hardcode boundaries?
    bins = np.linspace(0.4, 1.4, 21)
    plt.figure(figsize=(10, 10))
    target_energies = 2**np.linspace(8, 23, 16)
    ref_Etot = ref_eperlayers.sum(axis=-1)
    gen_Etot = gen_eperlayers.sum(axis=-1)
    for i in range(len(target_energies) - 1):
        if i > 3 and 'photons' in arg.dataset:
            bins = np.linspace(0.9, 1.1, 21)
        energy = target_energies[i]
        which_showers_ref = ((ref_einc.squeeze() >= target_energies[i]) & \
                             (ref_einc.squeeze() < target_energies[i+1])).squeeze()
        which_showers_hlf = ((gen_einc.squeeze() >= target_energies[i]) & \
                             (gen_einc.squeeze() < target_energies[i+1])).squeeze()
        ax = plt.subplot(4, 4, i+1)
        counts_ref, _, _ = ax.hist(ref_Etot[which_showers_ref] /\
                                   ref_einc.squeeze()[which_showers_ref],
                                   bins=bins, label='reference', density=True,
                                   histtype='stepfilled', alpha=0.2, linewidth=2., color='#5EC962')
        counts_data, _, _ = ax.hist(gen_Etot[which_showers_hlf] /\
                                    gen_einc.squeeze()[which_showers_hlf], bins=bins,
                                    label='generated', histtype='step', linewidth=3., alpha=1.,
                                    density=True, color='#3B528B')
        if i in [0, 1, 2]:
            energy_label = 'E = {:.0f} MeV'.format(energy)
