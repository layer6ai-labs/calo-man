...
from .high_level_features import HighLevelFeatures as HLF
from two_step_zoo.datasets.calo_challenge_showers import get_raw_data_arrays
from two_step_zoo.datasets.calo_challenge_epl import get_raw_data_arrays_epl
...
parser.add_argument('--mode', '-m', default='all',
                    ...
                    help=("What metric to evaluate: " +\
                          ...
                          "'cls-low' trains a classifier on the low-level features;" +\
                          "'cls-low-normed' trains a classifier on the low-level features with calorimeter layers;" +\
                          "'cls-high' trains a classifier on the high-level features;" +\
                          "'all' does the full evaluation, ie all of the above with low-level classifier."))
parser.add_argument('--dataset', '-d',
                    choices=['photons1', 'pions1', 'electrons2', 'electrons3'],
...
def calibrate_classifier(model, calibration_data, args=None):
...
def check_file(given_file, arg, which=None):
    ...
    num_features = {'photons1': 368, 'pions1': 533, 'electrons2': 6480, 'electrons3': 40500,
                    'electrons2-cart': 6480, 'electrons3-cart': 40500}[arg.dataset]
...
def get_particle_xml(dataset):
        particle = {'photons1': 'photon', 'pions1': 'pion',
                    'electrons2': 'electron', 'electrons3': 'electron'}[dataset]
        xml_filename = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))),
                                                    "evaluators", "binning", f"binning_dataset_{dataset}.xml")
        return particle, xml_filename
    
def evaluate_samples(gen_showers, gen_energies, cfg, logdir):
    # Similar to __main__
    output_dir = logdir + "/evaluation_results"
    if not os.path.isdir(output_dir):
        os.makedirs(output_dir)

    dataset = cfg["dataset"]
    particle, xml_filename = get_particle_xml(dataset)
    gen_hlf = HLF(particle, filename=xml_filename)
    ref_hlf = HLF(particle, filename=xml_filename)

    # load reference data from disk to ensure no lossy conversion is applied
    cfg_copy = deepcopy(cfg)
    cfg_copy["preprocess_physics_data"] = False
    data_root=cfg_copy.get("data_root", "data/")

    ref_showers, ref_energies = get_raw_data_arrays(cfg_copy, dataset, data_root, "test")
    ref_showers = np.reshape(ref_showers, (ref_showers.shape[0], -1))
    ref_hlf.Einc = ref_energies
    ...
    args.min_energy = {'photons1': 10, 'pions1': 10,
                    'electrons2': 0.5e-3/0.033, 'electrons3': 0.5e-3/0.033}[args.dataset]

    print("Plotting average shower next to reference...")
    plot_layer_comparison(gen_hlf, 
                          gen_showers.mean(axis=0, keepdims=True),
                          ref_hlf, 
                          ref_showers.mean(axis=0, keepdims=True), 
                          args)

    print("Plotting average showers")
    # get maximum energy so that colorbars can be synchronized
    vmax_gen = gen_showers.mean(axis=0).max()
    vmax_ref = ref_showers.mean(axis=0).max()
    vmax = max(vmax_gen, vmax_ref)
    try:
        _ = gen_hlf.DrawAverageShower(gen_showers, 
                                        filename=os.path.join(output_dir,
                                                            'average_shower_dataset_{}.png'.format(dataset)),
                                        title="Shower Average - Generated Samples",
                                        show=False,
                                        vmax=vmax)

        ref_hlf.avg_shower = ref_showers.mean(axis=0, keepdims=True)
        _ = ref_hlf.DrawAverageShower(ref_hlf.avg_shower,
                                        filename=os.path.join(output_dir,
                                                            'reference_average_shower_dataset_{}.png'.format(dataset)),
                                        title="Shower Average - Test Dataset",
                                        show=False,
                                        vmax=vmax)
        plt.close()
    except ValueError:
        print("Failed to plot all average showers")

    print("Plotting average showers at energies")
    try:
        if '1' in dataset:
        ...
        for i in range(len(target_energies)-1):
            filename = 'average_shower_dataset_{}_E_{}.png'.format(dataset, target_energies[i])
            which_gen_showers = ((gen_energies >= target_energies[i]) & (gen_energies < target_energies[i+1])).squeeze()
            which_ref_showers = ((ref_hlf.Einc >= target_energies[i]) & (ref_hlf.Einc < target_energies[i+1])).squeeze()
            vmax_gen = gen_showers[which_gen_showers].mean(axis=0).max()
            vmax_ref = ref_showers[which_ref_showers].mean(axis=0).max()
            vmax = max(vmax_gen, vmax_ref)
            _ = gen_hlf.DrawAverageShower(gen_showers[which_gen_showers],
                                        filename=os.path.join(output_dir, filename),
                                        title=plot_title[i] + ' - Generated Samples',
                                        show=False,
                                        vmax=vmax)

            ref_hlf.avg_shower_E = {}
            ref_hlf.avg_shower_E[target_energies[i]] = ref_showers[which_ref_showers].mean(axis=0, keepdims=True)
            _ = ref_hlf.DrawAverageShower(ref_hlf.avg_shower_E[target_energies[i]],
                                        filename=os.path.join(output_dir, 'reference_'+filename),
                                        title=plot_title[i] + ' - Test Dataset',
                                        show=False,
                                        vmax=vmax)
            plt.close()
    except ValueError:
        print("Failed to plot all showers")

    print("Evaluating histograms")
    try:
        gen_hlf.CalculateFeatures(gen_showers)
        gen_hlf.Einc = gen_energies
        ref_hlf.CalculateFeatures(ref_showers)
        with open(os.path.join(output_dir, 'histogram_chi2_{}.txt'.format(dataset)), 'w') as f:
            f.write('List of chi2 of the plotted histograms, see eq. 15 of 2009.03796 for its definition.\n')
        plot_histograms(gen_hlf, ref_hlf, args)
    except RuntimeError:
        print("Failed to plot all histograms")

    print("Training Classifier")
    source_path = Path(logdir) / "generated_showers.hdf5"
    ...
    filename = filenames[dataset][1]
    if not filename: # pions
        filename = filenames[dataset][0]
    reference_path = Path(data_root) / filename[0]
    reference_file = h5py.File(reference_path, 'r')

    try:
        if args.mode in ['all', 'cls-low']:
            args.mode = 'cls-low'
            source_array = prepare_low_data_for_classifier(source_file, gen_hlf, 0.,
                                                           normed=False)
            reference_array = prepare_low_data_for_classifier(reference_file, ref_hlf, 1.,
                                                              normed=False)
            ...
            source_array = prepare_high_data_for_classifier(source_file, gen_hlf, 0.)
            reference_array = prepare_high_data_for_classifier(reference_file, ref_hlf, 1.)

        train_data, test_data, val_data = ttv_split(source_array, reference_array)

        # set up args
        args.device = "cuda" if torch.cuda.is_available() else "cpu"
        args.cls_n_epochs = cfg.get("cls_n_epochs", 50)
        args.cls_lr = cfg.get("cls_lr", 2e-4)
        args.cls_batch_size = cfg.get("cls_batch_size", 1000)
        args.save_mem = True
        ...
        with torch.enable_grad():
            ...
            DNN_kwargs = {
                'num_layer': cfg.get("cls_n_layer", 2),
                'num_hidden': cfg.get("cls_n_hidden", 512),
                'input_dim': input_dim,
                'dropout_probability': cfg.get("cls_dropout_probability", 0.),
                }
            ...
            print("Classifier on {} features has {} parameters".format(args.mode, int(total_parameters)))
...
def evaluate_energies_per_layer(gen_eperlayers, gen_einc, cfg, logdir):
    output_dir = logdir + "/epl_evaluation_results"
    if not os.path.isdir(output_dir):
        os.makedirs(output_dir)
    ...
    ref_eperlayers, ref_energies = get_raw_data_arrays_epl(cfg_copy, dataset, data_root, "test")
    ref_eperlayers = np.reshape(ref_eperlayers, (ref_eperlayers.shape[0], -1))
    ...
    parser_replacement = {
        ...
        'output_dir': output_dir,
        'x_scale': 'log',
        "min_energy": cfg_copy["energy_min"],
        }
    args = argparse.Namespace(**parser_replacement)
    
    print("Plotting eperlayer distributions...")
    plot_eperlayer_comparison(gen_eperlayers, ref_eperlayers, args)
    plot_Etot_Einc_comparison(gen_eperlayers, gen_einc, ref_eperlayers, ref_energies, args)
    if "photons" in dataset or "pions" in args.dataset:
        plot_Etot_Einc_discrete_comparison(gen_eperlayers, gen_einc, ref_eperlayers, ref_energies, args)
...
if __name__ == '__main__':
    ...
    particle = {'photons1': 'photon', 'pions1': 'pion',
                'electrons2': 'electron', 'electrons3': 'electron'}[args.dataset]
    xml_filename = os.path.join(os.path.dirname(os.path.realpath(__file__)), "binning", f"binning_dataset_{args.dataset}.xml")
    # minimal readout per voxel, ds1: from Michele, ds2/3: 0.5 keV / 0.033 scaling factor
    args.min_energy = {'photons1': 10, 'pions1': 10,
                       'electrons2': 0.5e-3/0.033, 'electrons3': 0.5e-3/0.033}[args.dataset]
    hlf = HLF(particle,filename=xml_filename)
    ...
    if os.path.exists(os.path.join(args.source_dir, args.reference_file_name + '.pkl')):
        ...
        reference_hlf = HLF(particle, filename=xml_filename)
        reference_hlf.Einc = reference_energy
        save_reference(reference_hlf, os.path.join(args.source_dir, args.reference_file_name + '.pkl'))
    args.x_scale = 'log'
        ...
        _ = hlf.DrawAverageShower(shower,
                                  filename=os.path.join(args.output_dir,
                                                        'average_shower_dataset_{}.png'.format(
                                                            args.dataset)),
                                  title="Shower Average - Generate Samples")
