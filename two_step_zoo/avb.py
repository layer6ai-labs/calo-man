...
class AdversarialVariationalBayes(GeneralizedAutoEncoder, DensityEstimator):
    ...
    def encode_transformed(self, x, y):
        ...
        if self.cnn:
            return self.encoder(x, y, eps)
        else:
            return self.encoder(torch.cat((x, eps), 1), y)
    ...
    def _discr_error_batch(self, x, y):
        ...
        z_q = self.encode_transformed(x, y)
        ...
        d_z_q = self.discriminator(torch.cat((x_flat, z_q), 1), y)
        d_z_p = self.discriminator(torch.cat((x_flat, z_p), 1), y)
    ...
    def log_prob(self, x, y):
        ...
        # NOTE: Discriminator always is MLP so flatten inputs
        x_flat = x.flatten(start_dim=1)
        d_z = self.discriminator(torch.cat((x_flat, z), 1), y)
