...
class EnergyBasedModel(DensityEstimator):
    ...
    def _langevin_dynamics_step(self, x, y, step_size, sigma, grad_clamp):
        x.requires_grad = True
        out = self.energy_func(x, y)
        out.sum().backward()
    ...
    def _langevin_dynamics(self, x, y, steps, step_size, sigma, grad_clamp):
        ...
        for _ in range(steps):
            x = self._langevin_dynamics_step(x, y, step_size, sigma, grad_clamp)
    ...
    def sample(self, n_samples, y, steps=60, step_size=10, eps_new=0.0, sigma=0.005, grad_clamp=0.03,
        ...
        x = self._langevin_dynamics(x, y, steps, step_size, sigma, grad_clamp)
    ...
    def log_prob(self, x, y):
        ...
        return -self.energy_func(x, y)

    @batch_or_dataloader(agg_func=lambda x: torch.mean(torch.Tensor(x)))
    def loss(self, x, y):
        batch_size = x.shape[0]
        x = self._data_transform(x)
        pos = self.energy_func(x, y)
        samples = self.sample(n_samples=batch_size,
                              y=y,
                              steps=self.ld_steps,
                              ...
                              for_loss=True,
                              update_buffer=True)
        neg = self.energy_func(samples, y)
