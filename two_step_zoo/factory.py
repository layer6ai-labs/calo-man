...
from .networks import (
    ...
    CNN, T_CNN,
    GaussianMixtureLSTM,
    ResidualDecoder, ResidualEncoder, ParallelResidualDecoder, ParallelResidualEncoder,
    SimpleFlowTransform, SimpleAutoregressiveFlowTransform
)
...
def get_single_module(cfg, **kwargs):
    ...
    cfg["label_dim"] = kwargs.get("label_dim", None)
...
def get_avb_module(cfg):
    ...
    return AdversarialVariationalBayes(
        ...
        cnn=True if cfg["encoder_net"] in ("cnn", "residual") else False,
        **get_data_transform_kwargs(cfg)
    )
...
def get_flow_module(cfg):
    label_dim = cfg["label_dim"] if cfg.get("use_labels", True) else None
    ...
    if cfg["transform"] == "simple_nsf":
        transform = SimpleFlowTransform(
            ...
            context_features=label_dim,
            dropout_probability=cfg["dropout_probability"],
        )
    elif cfg["transform"] == "simple_a-nsf":
        transform = SimpleAutoregressiveFlowTransform(
            ...
            tail_bound=cfg["tail_bound"],
            context_features=label_dim,
            use_residual_blocks=cfg["use_residual_blocks"],
    ...
    elif cfg["base_distribution"] == "conditional_diagonal":
        encoder = MLP(1, [64, 64], cfg["data_dim"] * 2, nn.ReLU)
        base_distribution = ConditionalDiagonalNormal(
            shape=[cfg["data_dim"]],
...
def get_ebm_module(cfg):
    ...
    label_dim = cfg["label_dim"] if cfg.get("use_labels", True) else 0

    if cfg["net"] == "mlp":
        if label_dim:
            energy_func = ParallelMLP(
                input_x_dim=cfg["data_dim"],
                input_y_dim=label_dim,
        ...
        energy_func = CNN(
            input_channels=cfg["data_shape"][0]+label_dim,
...
def get_encoder_decoder(cfg):
    ...
    if cfg["encoder_net"] == "mlp":
        if label_dim:
            encoder = ParallelMLP(
                input_x_dim=cfg["data_dim"]+cfg.get("noise_dim", 0),
                input_y_dim=label_dim,
        ...
        if label_dim:
            input_shape = list(cfg["data_shape"])
            input_shape[1] = input_shape[1] + 2*cfg.get("pad_wrap", 0)

            encoder_class = ParallelResidualEncoder
            kwargs = {"input_shape": input_shape}
        else:
            encoder_class = ResidualEncoder
            kwargs = {}
    ...
    if cfg["decoder_net"] == "mlp":
        ...
        decoder = T_CNN(
            input_dim=cfg["latent_dim"]+label_dim,
        ...
        decoder_class = ParallelResidualDecoder if label_dim else ResidualDecoder
        decoder = decoder_class(
            ...
            output_channels=decoder_output_channels,
            output_split_sizes=decoder_output_split_sizes,
            image_height=cfg["data_shape"][1] + 2*cfg.get("pad_wrap", 0),
...
def get_discriminator(cfg):
    ...
    if cfg.get("use_labels", True):
        net = ParallelMLP(
                input_x_dim=cfg["latent_dim"]+extra_dims,
                input_y_dim=cfg["label_dim"],
...
def get_data_transform_kwargs(cfg):
    return {
        ...
        "pad_wrap": cfg.get("pad_wrap", None),
        ...
        "use_labels": cfg.get("use_labels", True),
    }
