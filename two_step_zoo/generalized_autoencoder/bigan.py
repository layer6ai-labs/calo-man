...
class BiGAN(GeneralizedAutoEncoder):
    ...
    def train_batch(self, x, y, **kwargs):
        ...
        discriminator_loss = self._discr_error_batch(x, y).mean()
        ...
        if self.step_count >= self.num_discriminator_steps:
            ...
            generator_encoder_loss = self._ge_error_batch(x, y).mean()
    ...
    def _discriminator_outputs(self, x, y):
        x = self._data_transform(x)
        z_e = self.encode_transformed(x, y)

        # sample from latent prior and decode (generate)
        z_p = uniform_sample(template=z_e, range=self.uniform_sample_range)
        x_g = self.decode_to_transformed(z_p, y)

        # NOTE: Discriminator always is MLP so flatten inputs
        x_flat = x.flatten(start_dim=1)
        x_g_flat = x_g.flatten(start_dim=1)
        d_z_g = self.discriminator(torch.cat((x_g_flat, z_p), 1), y)
        d_z_e = self.discriminator(torch.cat((x_flat, z_e), 1), y)

        return d_z_g, d_z_e, x, x_g, z_e, z_p

    def _discr_error_batch(self, x, y):
        d_z_g, d_z_e, x_true, x_fake, z_true, z_fake = self._discriminator_outputs(x, y)

        if self.wasserstein and self.gradient_penalty:
            discriminator_loss = -torch.mean(d_z_e) + torch.mean(d_z_g) +\
                                 self._grad_penalty(x_true, x_fake, z_true, z_fake, y)
    ...
    def _ge_error_batch(self, x, y):
        # Reconstruction loss
        rec_loss, z_q = self.rec_error(x, y, return_z=True)
        
        # Discriminator loss
        d_z_g, d_z_e = self._discriminator_outputs(x, y)[0:2]
    ...
    def _grad_penalty(self, x_true, x_fake, z_true, z_fake, y):
        ...
        d_x = self.discriminator(d_input, y)
